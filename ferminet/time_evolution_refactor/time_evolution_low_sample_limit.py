...
def make_td_opt_update_step_full_solve(
  ...
  def accumulate_samples(
    ...
    def compute_SR_chunked(params, data, chunk_size=256):
        ...
        O_minibatch = jnp.zeros((Np, Ns_minibatch), dtype=float)
    ...
    return loss, aux_data, grad_vector, O

  def conduct_timestep(
      params, grad_vector, O
  ):
    
    flat_params, unravel_fn = jax.flatten_util.ravel_pytree(params)
    n_params = flat_params.shape[0]
    
    O_alpha = jnp.mean(O, axis=1)
    O = O - O_alpha
    O = O / jnp.sqrt(O.shape[1])
    fisher = O * jnp.conjugate(O)
  ...
  def step(
    ...
    def accumulate_samples_inner_fn(i, carry):
      ...
      loss, aux_data, grad_vector, fisher = accumulate_samples(
        params, loss_key, data, time, grad_vector, fisher, i)
    ...
    def rk2_inner_fn(params, key, data, time):
      ...
      O = jnp.zeros((n_params, n_samples))
